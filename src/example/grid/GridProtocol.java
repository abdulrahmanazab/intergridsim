 ...
 */
public class GridProtocol extends IdleProtocol implements Protocol, Linkable
{
...
public Object clone()
{
	GridProtocol ip = null;
	try { ip = (GridProtocol) super.clone(); }
...
public boolean removeNeighbour(Node n){
    ...
    for(Node item : neighbors)
        if(n.getIndex() != item.getIndex())
            result.add(item);
        else
            found = true;
        
    neighbors = (Node[]) result.toArray(neighbors);
...
//public Vector<GridNodeResourceState> ShareData()
}
